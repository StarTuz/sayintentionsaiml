/*
 * SayIntentions.AI X-Plane Adapter Plugin
 *
 * This plugin reads X-Plane DataRefs and writes them to a JSON file
 * for consumption by the SayIntentions native client (Linux/Mac).
 *
 * It also reads commands from the client (via JSONL file) and applies
 * them back to the simulator.
 *
 * Build: See CMakeLists.txt
 * Install: Copy to X-Plane 12/Resources/plugins/SayIntentionsAIml/
 */
...
/* Plugin metadata */
#define PLUGIN_NAME "SayIntentions.AI Adapter"
#define PLUGIN_SIG "community.sayintentionsml.xplane.adapter"
#define PLUGIN_DESC "Bridges X-Plane to SayIntentions.AI native client"
...
static void LogOpen(void) {
  ...
  if (g_log_fp) {
    ...
    fprintf(g_log_fp, "\n=== SayIntentionsAIml Session Started: %s ===\n",
            time_str);
    fflush(g_log_fp);
  }
}
...
static void InitFilePaths(void) {
...
#if IBM
  /* Windows: Use %LOCALAPPDATA%\SayIntentionsAI */
  const char *localappdata = getenv("LOCALAPPDATA");
  if (localappdata) {
    snprintf(g_data_dir, sizeof(g_data_dir), "%s" PATH_SEP "SayIntentionsAI",
             localappdata);
  } else {
    snprintf(g_data_dir, sizeof(g_data_dir), "C:" PATH_SEP "SayIntentionsAI");
  }
#elif APL
  /* macOS: Use ~/Library/Application Support/SayIntentionsAI */
  ...
  if (home) {
    snprintf(g_data_dir, sizeof(g_data_dir),
             "%s/Library/Application Support/SayIntentionsAI", home);
  } else {
    snprintf(g_data_dir, sizeof(g_data_dir), "/tmp/SayIntentionsAI");
  }
#else
  /* Linux: Use ~/.local/share/SayIntentionsAI */
  const char *home = getenv("HOME");
  if (!home) {
    struct passwd *pw = getpwuid(getuid());
    if (pw)
  ...
  if (home) {
    snprintf(g_data_dir, sizeof(g_data_dir), "%s/.local/share/SayIntentionsAI",
             home);
  } else {
    snprintf(g_data_dir, sizeof(g_data_dir), "/tmp/SayIntentionsAI");
  }
  ...
  snprintf(g_log_file, sizeof(g_log_file),
           "%s" PATH_SEP "sayintentionsaiml.log", g_data_dir);
}
